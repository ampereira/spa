...
namespace Hardware {
	...
	bool EventSet::create (vector<string> evts) {
		...
		while (it < size) {
			...
			switch (retval) {
				case PAPI_OK 	  : cout << "PAL | EventSet: Counter " << event_list[it++].getName() << " added successfully." << endl;
									break;
				case PAPI_ECNFLCT : conflict = true; break;
				default : error = PAPI_strerror(retval);
						  cerr << "PAL | EventSet: error adding counter " << event_list[it].getName() << " to eventset - " << error << endl;
						  cerr << "PAL | EventSet: library will exit." << endl;
						  return false;
			}
		}
	...
	Event EventSet::getEvent (unsigned index) {
		return event_list[index];
	}

	unsigned EventSet::numberOfEvents (void) {
		return total_events;
	}

	void EventSet::addResult(unsigned index, long long unsigned value) {
